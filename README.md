...
```javascript
<BaseComponent render={(props) => <ChildComponent {...props}/>} />
```
...
##### 错误的边界 Error boundAries

- 部分UI中的JavaScript错误不应该破坏整个应用程序
- 解决React用户的这问题，React16引入了 错误边界 的概念
- 错误边界 是一个组件 可以用来捕获它的子组件中产生的错误，纪录错误日志并在错误发生的时没展示一个“回退” 或者说是一个错误信息页面，以避免因为局部组件错误而导致整个组件树的崩溃
- 整个组件是所有子组件发送错误的捕获者吗，所有子组件的错误到达错误边界组件后，错误信息被拦截并不再向上冒泡，所以这个组件就是错误的一个边界
- 原则上来讲，错误边界是用来保证React正常渲染UI的，而不是真的用来捕获异常的
- 自React16 ，任何未被错误边界捕获的错误将会卸载整个React组件树




**Suspense** 可以用来实现同步网络请求并渲染
